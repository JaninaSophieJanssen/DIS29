<script>
    ...
    unter_fach_list = unter_fach_list
}
...
<div class="topLevelContainer">
    <div class="topLevelTopic">
        ...
        <button class="image_container" on:click={onClickThemen}>
            <img src={!themengebiet_open ? arrow_down : arrow_up} alt="arrow_down" class="image" />
        </button>
    </div>
    {#if data["Unterthemen"] && themengebiet_open}
        {#each data["Unterthemen"] as unterthema}
            <div class="secondLevelContainer">
                <div class="topLevelTopic">
                    <p class="fachTitle">
                        {unterthema["Unterthema"]}
                    </p>
                    <button class="image_container" on:click={() => onClickFach(unterthema["Unterthema"])}>
                        <img src={!fach_list.includes(unterthema["Unterthema"]) ? arrow_down : arrow_up} alt="arrow_down" class="image" />
                    </button>
                </div>
                {#if unterthema["Unter-Unterthemen"] && fach_list.includes(unterthema["Unterthema"])}
                    {#each unterthema["Unter-Unterthemen"] as UnterUnterthema}
                        <div class="thirdLevelContainer">
                            <div class="topLevelTopic">
                                <p class="type_title">
                                    {UnterUnterthema["Name"]}
                                </p>
                                <button class="image_container" on:click={() => onClickUnterFach(UnterUnterthema["Name"])}>
                                     <img src={!unter_fach_list.includes(UnterUnterthema["Name"]) ? arrow_down : arrow_up} alt="arrow_down" class="image" />
                                </button>
                            </div>
                            {#if UnterUnterthema["Professoren"] && unter_fach_list.includes(UnterUnterthema["Name"])}
                                {#each UnterUnterthema["Professoren"] as prof}
                                    <div class="fourthLevelContainer">
                                        <div class="topLevelTopic">
                                            <p class="type_title">
                                                {prof["Name"]}
                                            </p>
                                            <button class="image_container" on:click={() => onClickProf(prof["Name"])}>
                                                <img src={!prof_list.includes(prof["Name"]) ? arrow_down : arrow_up} alt="arrow_down" class="image" />
                                            </button>
                                        </div>
