<!-- SearchMask.svelte -->
<script lang="ts">
    import { personenArray, themengebieteArray, faecherArray, _data, schlagwoerterArray, unterFaecherArray } from "../data/data";
    import AutocompleteItem from './AutocompleteItem.svelte';
  
    export let searchTerm: string | Record<string, any>[] = '';
 
    let inputValue = searchTerm as string;
  
    let filteredExperts: string[] = [];
    let rawResultExperts: string[] = [];
    let experts: string[] = personenArray.concat(themengebieteArray, faecherArray, unterFaecherArray, schlagwoerterArray);
  
    let searchInput: HTMLInputElement | null;
    let highlightedItemIndex: number | null = null;
  
    const getAllSwForProf = (profId: string): string[] => {
      let allSchlagworte: string[] = [];
    ...
    const getProfessorDetailsFromId = (searchId: string) => {
      let professorDetails: Record<string, any> = {};
    ...
    const calculateSearchResultObject = (searchArray: string[]): Record<string, any>[] => {
      let resultObjectArray: Record<string, any>[] = [];
    ...
    const getPersonIdsFromThemengebiet = (searchedThemengebiet: string): string[] => {
    ...
    const getPersonIdsFromFach = (searchedFach: string): string[] => {
      let personIds: string[] = [];
    ...
    const getPersonIdsFromPerson = (searchedPerson: string): string[] => {
      let personIds: string[] = [];
    ...
    const getPersonIdsFromSchlagwort = (searchedSchlagwort: string): string[] => {
      let personIds: string[] = [];
    ...
    const getExpertIds = (searchTermArray: string[]): string[] => {
      let foundIds: string[] = [];
    ...
    const setInputAndSubmit = (expert: string) => {
  submitSearch();
  // Setze die vorgeschlagenen Wörter nur mit dem ausgewählten Wort
  ...
  filteredExperts = [];
    ...
    const makeMatchingCharsBold = (str: string): string => {
      let re = new RegExp(inputValue, 'gi');
      let matched = str.match(re);
      if (matched) {
        let boldWrappedMatch = `<strong>${matched.join('')}</strong>`;
        return str.replace(re, boldWrappedMatch);
      }
      return str;
    };
  
    const removeBoldTags = (str: string): string => {
      return str.replace(/<(.)*?>/g, "");
    };
  
    const navigateWithArrowKeys = (event: KeyboardEvent) => {
      if (event.key === "ArrowDown" && highlightedItemIndex !== null && highlightedItemIndex <= filteredExperts.length - 1) {
  ...
  <form autocomplete="off" on:submit|preventDefault={submitSearch}>
    ...
    <ul id="autocomplete-items-list">
      ...
      <AutocompleteItem itemLabel={expert} selected={i === highlightedItemIndex} on:click={() => {setInputAndSubmit(expert)}}/>
      {/each}
    </ul>
    {/if}
  </form>
